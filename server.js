...
async function computeMarkets(stake){
  ...
  async function runChunk(chunk){
              ...
              const prompt=`You verify candidate matches between Polymarket and Kalshi prediction markets. For each candidate, decide if BOTH markets resolve to the SAME outcome on the SAME timeline.\n\nSTRICT RULES — reject the match if ANY of these is true:\n1. The resolution dates/deadlines differ (e.g. "2026 season" vs "before 2031" → REJECT, even if one is a subset of the other).\n2. The subject is broader on one side (e.g. "Team X wins" vs "any Canadian team wins" → REJECT).\n3. The thresholds, conditions, or scope differ in any way.\n4. One market resolves earlier than the other so capital is locked at different times.\n\nONLY accept matches where the resolution criteria are IDENTICAL: same person/team, same event, same exact deadline, same threshold. Both YES legs must pay out together AND both NO legs must pay out together at the same time.\n\nCANDIDATES:\n${lines}\n\nReply with ONLY valid JSON, no prose:\n{"matches":[{"i":<candidate index>,"why":"brief reason confirming identical timeline+scope"}]}\nInclude only verified matches. If none, return {"matches":[]}.`;
              try{
