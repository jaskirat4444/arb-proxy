...
fetch("https://trading-api.kalshi.com/trade-api/v2/markets?status=open&limit=50&series_ticker=NBA",{headers:{"Authorization":`Bearer ${KKEY}`}})
]);
...
const ai=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":KEY,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:"claude-haiku-4-5-20251001",max_tokens:1000,messages:[{role:"user",content:`Match NBA prediction markets on the IDENTICAL game and outcome. POLYMARKET:${poly.map((m,i)=>`[${i}]${m.title} YES=${Math.round(m.yes*100)}c`).join("|")} KALSHI:${kalshi.map((m,i)=>`[${i}]${m.title} YES=${Math.round(m.yes*100)}c`).join("|")} Reply ONLY with JSON:{"matches":[{"pi":0,"ki":0,"note":"reason"}]}`}]})});
